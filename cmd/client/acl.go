...
Examples:
  ...
  knox acl get myapp:api_key --json`,
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			logger.Printf("ACL for key: %s\n\n", keyID)

			if len(*acl) == 0 {
				logger.Println("No ACL entries")
				return nil
			}

			w := tabwriter.NewWriter(os.Stdout, 0, 0, 2, ' ', 0)
			logger.Println("TYPE\tPRINCIPAL\tACCESS")
			logger.Println("----\t---------\t------")
...
Examples:
  ...
  knox acl add myapp:api_key UserGroup:developers:Admin`,
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			logger.Success(fmt.Sprintf("ACL entry added to %s", keyID), map[string]any{
				"key_id":         keyID,
				"principal":      acl[0].ID,
				"principal_type": formatPrincipalType(acl[0].Type),
				"access_type":    formatAccessType(acl[0].AccessType),
			})
...
Examples:
  ...
  knox acl remove myapp:api_key Service:spiffe://example.com/myservice`,
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			logger.Success(fmt.Sprintf("ACL entry removed from %s", keyID), map[string]any{
				"key_id":         keyID,
				"principal":      acl[0].ID,
				"principal_type": formatPrincipalType(acl[0].Type),
			})

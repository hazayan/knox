...
Examples:
  ...
  knox key create myapp:api_key --data "secret" --acl "User:alice@example.com:Read"`,
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			logger.Success(fmt.Sprintf("Key created: %s", keyID), map[string]any{
				"key_id":     keyID,
				"version_id": versionID,
			})
			return nil
  ...
  knox key get myapp:api_key --status Primary`,
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			if versionStatus != "" {
				...
				return displayKey(os.Stdout, key, showAll)
			}
...
Examples:
  ...
  knox key list --json`,
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			if len(keys) == 0 {
				logger.Println("No keys found")
				return nil
			}

			logger.Printf("Found %d key(s):\n\n", len(keys))
			for _, key := range keys {
				logger.Printf("  %s\n", key)
			}

			logger.Debug("Key list operation completed", map[string]any{
				"key_count": len(keys),
			})
			return nil
		},
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			if !force {
				...
				if strings.ToLower(response) != "y" {
					...
					logger.Debug("Key deletion cancelled by user", map[string]any{
						"key_id": keyID,
					})
					return nil
				}
			...
			logger.Success(fmt.Sprintf("Key deleted: %s", keyID), map[string]any{
				"key_id": keyID,
			})
			return nil
		},
...
Examples:
  ...
  echo "newsecret123" | knox key rotate myapp:api_key`,
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			logger.Success(fmt.Sprintf("Key rotated: %s", keyID), map[string]any{
				"key_id":     keyID,
				"version_id": versionID,
			})
...
func displayKey(w io.Writer, key *types.Key, showAll bool) error {
	if jsonOutput {
	...
	if showAll {
		...
		fmt.Fprintln(w, string(primary.Data))
	}
...
func parseACL(entries []string) (types.ACL, error) {
	...
	for _, entry := range entries {
		// Split by colon, but handle cases where ID contains colons (like SPIFFE URLs)
		...
		if lastColon == firstColon || lastColon == len(entry)-1 {
			return nil, fmt.Errorf("invalid ACL entry format: %s (expected TYPE:ID:ACCESS)", entry)
		}

		principalTypeStr := entry[:firstColon]
		id := entry[firstColon+1 : lastColon]
		accessTypeStr := entry[lastColon+1:]

		// Parse principal type
		var principalType types.PrincipalType
		switch strings.ToLower(principalTypeStr) {
		case "user":
		...
		default:
			return nil, fmt.Errorf("invalid principal type: %s", principalTypeStr)
		}

		// Parse access type
		var accessType types.AccessType
		switch strings.ToLower(accessTypeStr) {
		case "read":
		...
		case "none":
			accessType = types.None
		default:
			return nil, fmt.Errorf("invalid access type: %s", accessTypeStr)
		}
...
// Global variable for mock client injection in tests.
var (
	mockAPIClient client.APIClient
	mockMutex     sync.RWMutex
)

// SetMockAPIClient sets a mock client for testing.
func SetMockAPIClient(client client.APIClient) {
	mockMutex.Lock()

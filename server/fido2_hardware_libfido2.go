...
type hardwareFido2Credential struct {
	CredentialID []byte
	AuthData     []byte
}

type hardwareFido2Assertion struct {
	AuthData  []byte
	Signature []byte
}

func hardwareFido2FirstDevicePath() (string, error) {
	C.fido_init(0)
	var cPath *C.char
	rc := C.knox_fido2_first_device_path(&cPath)
	if rc != C.FIDO_OK {
		return "", hardwareFido2Error("discover FIDO2 device", rc)
	}
	defer C.free(unsafe.Pointer(cPath))
	return C.GoString(cPath), nil
}

func hardwareFido2MakeCredential(devicePath, rpID, rpName, pin string, userID []byte, userName, displayName string, clientData []byte) (hardwareFido2Credential, error) {
	C.fido_init(0)
	...
	if rc != C.FIDO_OK {
		return hardwareFido2Credential{}, fmt.Errorf("%w (step %d)", hardwareFido2Error("make FIDO2 credential", rc), int(failedStep))
	}
	defer C.knox_fido2_free_buf(&credentialID)
	defer C.knox_fido2_free_buf(&authData)

	return hardwareFido2Credential{
...
func hardwareFido2GetAssertion(devicePath, rpID, pin string, credentialID []byte, clientData []byte) (hardwareFido2Assertion, error) {
	C.fido_init(0)
	clientDataHash := sha256.Sum256(clientData)
	cDevice := C.CString(devicePath)
	cRPID := C.CString(rpID)
	...
	if rc != C.FIDO_OK {
		return hardwareFido2Assertion{}, hardwareFido2Error("get FIDO2 assertion", rc)
	}
	defer C.knox_fido2_free_buf(&authData)
	defer C.knox_fido2_free_buf(&signature)
	defer C.knox_fido2_free_buf(&returnedUserID)

	return hardwareFido2Assertion{
		AuthData:  C.GoBytes(unsafe.Pointer(authData.ptr), C.int(authData.len)),
		Signature: C.GoBytes(unsafe.Pointer(signature.ptr), C.int(signature.len)),
	}, nil
}

func hardwareFido2Error(operation string, rc C.int) error {
